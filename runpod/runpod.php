...
final class RunpodTestRunner
{
    ...
    private function loadPods(): void
    {
        ...
        if (($this->selected_pod_url ?? '') !== '' && ($this->selected_model_id ?? '') !== '') {
            $this->pods[] = ['url' => $this->selected_pod_url, 'model_id' => $this->selected_model_id];
        } else {
            ...
            foreach ($blocks as $block) {
                ...
                if (!empty($url[0]) && !empty($model[1]) && $model[1] !== 'none') {
                    $this->pods[] = ['url' => $url[0], 'model_id' => $model[1]];
                }
            }
        }
    ...
    private function buildPrompts(array $pod): array
    {
        ...
        return [
            ...
            [
                ...
                Stelle mit create_directory sicher, dass der Ordner /host/tmp existiert. Verwende dafuer nicht list_directory.
                Öffne den Browser und recherchiere auf https://en.wikipedia.org/wiki/List_of_filename_extensions_(A%E2%80%93E).
                Verwende genau 5 konkrete Dateiendungen von dieser Seite als Beispiele.
                ...
                $document_path .
                '".
                Verwende keinen anderen Dateipfad und insbesondere nicht /app/server oder /host/tmp fuer das Word-Dokument.
                Schließe den Browser wieder.
                ...
                Suche dabei gezielt nach genau diesem Betreff.
                Verwende fuer fetch_mails keinen Filterparameter, sondern hole die neuesten E-Mails aus INBOX und prüfe dann die passende Nachricht.
                Behandle die id aus fetch_mails als undurchsichtigen exakten String und verwende exakt diesen String unveraendert.
                Wenn du die passende E-Mail in fetch_mails gefunden hast, musst du danach zwingend view_mail fuer genau diese Nachricht aufrufen, um den Anhang zu prüfen.
                Rufe fetch_mails hoechstens einmal und view_mail hoechstens einmal auf.
                Verwende fuer view_mail ausschliesslich exakt die id aus fetch_mails.
                Leite niemals eine andere id aus dem EML-Inhalt, aus Headern oder aus Werten in spitzen Klammern ab.
                Beurteile den Anhang nur anhand der strukturierten view_mail-Antwort und ignoriere EML-Rohdaten fuer die id-Ermittlung.
                Sobald view_mail erfolgreich ausgefuehrt wurde und du den Anhang beurteilen kannst, ist die Aufgabe beendet und du darfst keine weiteren Tools mehr aufrufen.
                Nach einem erfolgreichen view_mail darfst du die ID nicht erneut interpretieren, nicht erneut aus dem Rohinhalt ableiten und kein zweites view_mail ausloesen.
    ...
    private function runPodTestBatch(array $pod): array
    {
        ...
        foreach ($prompts as $prompt) {
            ...
            echo 'ℹ️ Testing ' . $pod['model_id'] . ' (' . ($uses_mcp ? 'with MCP' : 'without MCP') . ')...' . PHP_EOL;
            echo '----------------------------------------' . PHP_EOL;
            __log_begin();
